...
class DataPreprocessor:
    def __init__(self, data_dir: Path):
        ...
        self.preped_dir = self.data_dir / 'preped'
        os.makedirs(self.preped_dir, exist_ok=True)
    ...
    def prep_naver_data(self, sampling_rate: float = 1.0) -> pd.DataFrame:
        ...
        self.output_file = self.preped_dir / f'naver_movie_review_sampling_{sampling_rate_str}.csv'
        if os.path.exists(self.output_file):
            print(f'{self.output_file} 파일이 이미 존재합니다.')
            self.data = pd.read_csv(self.output_file)
            return

        print(f'===네이버 영화 리뷰 데이터 로드 시작...\n Sampling Rate: {sampling_rate}')
        data_naver_dir = self.data_dir / 'naver_movie_review'
        ...
        data_file = self.data_dir / 'naver_movie_review.csv'
        naver_df.to_csv(data_file, index=False, encoding='utf-8')
        print(naver_df.head())
        self.data = naver_df
    ...
    def preprocess(self):
        ...
        # 저장
        self.prep_file = self.preped_dir / f'preped_{self.output_file.name}'
        self.data.to_csv(self.prep_file, index=False)
        print(f"전처리된 데이터를 {self.prep_file}에 저장했습니다.")
